import { useTransactionsByCharge } from "@/hooks/transactionsHooks/useTransactionsByCharge";
import { Charge, Transaction } from "@/types/types";
...
type TransactionsByChargeProps = {
  chargeId: Charge['_id'];
};

export default function TransactionsByCharge({chargeId}: TransactionsByChargeProps) {
  const { data, isLoading, isError } = useTransactionsByCharge(chargeId);
