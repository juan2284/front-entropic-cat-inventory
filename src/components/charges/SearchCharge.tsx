...
type SearchChargeProps = {
  charges: Charges;
  dispatch: Dispatch<ListActions>;
};

export default function SearchCharge({charges, dispatch}: SearchChargeProps) {
  const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
    const search = e.target.value;
    dispatch({ type: 'search-charge', payload: { search, data: charges } });
  };
  return (
    <nav className="w-full border-b border-b-gray-200 p-4 flex flex-row justify-between items-center">
      <h4 className="text-2xl font-light font-oswald text-gray-600">Pagos</h4>
