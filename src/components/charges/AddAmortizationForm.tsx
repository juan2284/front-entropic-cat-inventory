...
export default function AddAmortizationForm({charge, register, errors, getValues, setValue, setPending}: AddAmortizationFormProps) {
  const handleCalc = () => {
    const pendingAmount = Number(charge.pending_amount) - (Number(getValues().amount_one) + Number(getValues().amount_three) + (Number(getValues().amount_two) * Number(getValues().currency_rate)));
