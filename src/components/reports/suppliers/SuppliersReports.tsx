...
export default function SuppliersReports() {
  const { data, isLoading, isError, paginationData, suppliersPaginated } = useSuppliers();
  const handleDownloadCSV = () => {
    data?.unshift(paginationData.headersTable);
    exportCSVFile(data!, 'ReporteProveedores');
  };
    ...
    <>
      <section className="p-4">
        <header className="w-full flex md:flex-row justify-center items-center gap-2 mb-4 border-b border-b-gray-200">
          ...
          >Reporte de Proveedores <TruckIcon className="w-6 h-6 text-indigo-600" /></h3>
        </header>

        <main className="flex flex-col justify-center items-center gap-2">
          <PaginationReports paginationData={paginationData} dataName="Proveedores Registrados" />

          <SuppliersReportTable suppliers={suppliersPaginated!} />
