...
import RemindersReportTable from "@/components/reports/reminders/RemindersReportTable";
import Logo from "@/components/globals/Logo";

export default function RemindersReports() {
  const { data, isLoading, isError, paginationData, reminders } = useReminders();
  const handleDownloadCSV = () => {
    paginationData.remindersData?.unshift(paginationData.headersTable);
    exportCSVFile(paginationData.remindersData!, 'ReporteRecordatorios');
  };
    ...
    <>
      <section className="p-4">
        <header className="w-full flex md:flex-row justify-center items-center gap-2 mb-4 border-b border-b-gray-200">
          ...
          >Reporte de Recordatorios <BellAlertIcon className="w-6 h-6 text-indigo-600" /></h3>
        </header>

        <main className="flex flex-col justify-center items-center gap-2">
          <PaginationReports paginationData={paginationData} dataName="Recordatorios Registrados" />

          <RemindersReportTable reminders={reminders!} />
