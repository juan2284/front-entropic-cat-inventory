...
import ChargesReportTable from "./ChargesReportTable";
import Logo from "@/components/globals/Logo";

export default function ChargesReports() {
  const { data, isLoading, isError, paginationData, charges } = useCharges();
  const handleDownloadCSV = () => {
    paginationData.chargesData?.unshift(paginationData.headersTable);
    exportCSVFile(paginationData.chargesData!, 'ReportePagosProveedores');
  };

  if (isError) return (
    <div className="w-full h-screen flex flex-col justify-center items-center bg-white bg-opacity-15">
      <Logo />
      <h4 className="text-xs font-roboto text-gray-600 font-bold mt-2">No hay Pagos Registrados</h4>
    ...
    <>
      <section className="p-4">
        <header className="w-full flex md:flex-row justify-center items-center gap-2 mb-4 border-b border-b-gray-200">
          ...
          >Reporte de Pagos <CreditCardIcon className="w-6 h-6 text-indigo-600" /></h3>
        </header>

        <main className="flex flex-col justify-center items-center gap-2">
          <PaginationReports paginationData={paginationData} dataName="Compras Registradas" />

          <ChargesReportTable charges={charges!} />
