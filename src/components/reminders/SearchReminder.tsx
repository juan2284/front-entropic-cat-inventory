...
export default function SearchReminder({reminders, dispatch}: SearchReminderProps) {
  const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
    const search = e.target.value;

    dispatch({ type: 'search-reminder', payload: { search, data: reminders } });
  };
  return (
    <>
      <nav className="w-full border-b border-b-gray-200 p-4 flex flex-row justify-between items-center">
        <h4 className="text-2xl font-light font-oswald text-gray-600">Recordatorios</h4>
