...
export default function SelectFilterModal({customers, state, dispatch}: SelectFilterModalProps) {
  const { register, handleSubmit, reset} = useForm<Customer>();
  const handleForm = (data: Customer) => {
    dispatch({type: 'filter-customers', payload: {customers, filters: data}});
    reset();
  };
  return (
    <>
