...
export default function SearchStock({ stocks, dispatch}: SearchStockProps) {
  const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
    const search = e.target.value;
    dispatch({ type: 'search-stock', payload: { search, data: stocks } });
  };
  return (
    <>
      <nav className="w-full border-b border-b-gray-200 p-4 flex flex-row justify-between items-center">
        <h4 className="text-2xl font-light font-oswald text-gray-600">Inventarios</h4>
