...
export default function SearchTransaction({transactions, dispatch}: SearchTransactionProps) {
  const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
    const search = e.target.value;
    dispatch({ type: 'search-transaction', payload: { search, data: transactions } });
  };
  return (
    <>
      <nav className="w-full border-b border-b-gray-200 p-4 flex flex-row justify-between items-center">
        <h4 className="text-2xl font-light font-oswald text-gray-600">Transacciones</h4>
        ...
        <div className="w-full md:w-1/3">
          <div className="flex flex-row justify-center items-center">
            ...
            <input
              ...
              placeholder={`Buscar por ID del Pago o Cobro`}
