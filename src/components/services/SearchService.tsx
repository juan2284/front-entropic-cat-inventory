...
export default function SearchService({dispatch, services}: SearchServiceProps) {
  const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
    const search = e.target.value;

    dispatch({ type: 'search-service', payload: { search, data: services } });
  };
  return (
    <>
      <nav className="w-full border-b border-b-gray-200 p-4 flex flex-row justify-between items-center">
        <h4 className="text-2xl font-light font-oswald text-gray-600">Servicios</h4>
