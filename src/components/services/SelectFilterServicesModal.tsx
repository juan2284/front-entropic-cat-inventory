...
import { ServicesFilterView, Services } from "@/types/types";
import { contactResultsTranslation } from "@/utils/es";
import { Dialog, Transition } from "@headlessui/react";
import { Dispatch, Fragment } from "react";
import { useForm } from "react-hook-form";

type SelectFilterServicesModalProps = {
  state: {
    show: boolean
  },
  services: Services;
...
export default function SelectFilterServicesModal({state, services, dispatch}: SelectFilterServicesModalProps) {
  const { register, handleSubmit, reset } = useForm<ServicesFilterView>();
  const results = Object.keys(contactResultsTranslation);

  const handleForm = (data: ServicesFilterView) => {
    dispatch({ type: 'filter-services-view', payload: { services, filters: data } });
    reset();
  };
  return (
    <>
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              ...
              >
                <Dialog.Panel className="w-full rounded-sm transform overflow-hidden bg-white text-left align-middle shadow-xl transition-all p-2">
                  ...
                  >Filtrar Servicios por:</Dialog.Title>
                  ...
                  >
                    <div className="w-full text-center flex flex-row justify-between items-center gap-2">
                      ...
                      <div className="w-full flex flex-col justify-center items-center">
                        <label htmlFor="vehicle" className="w-full text-sm font-bold text-slate-700 gap-2 p-1">
                          Vehículo
                        </label>
                        <input
                          type="text"
