...
export default function SupplierAccountView() {
  ...
  const { data, isLoading, isError } = useSupplierById(params.supplierId!);
  const { data: chargesData, isError: chargeError, isLoading: chargeLoading, totalAmountPending } = useChargesBySupplier(params.supplierId!);
  const charges: Charges | undefined = chargesData?.filter(charge => charge.status === 'pending');
  const date = new Date();

  if (isError || chargeError) return (<h4 className="text-xs font-roboto text-gray-600 font-bold">Error con la Base de Datos</h4>);
  if (isLoading || chargeLoading) return (
        ...
        <Document>
          <Page size="A4" style={styles.page}>
            <View style={styles.section}>
              ...
              {/* Table */}
              {charges?.map(charge => (
                <View style={{ width: '100%', padding: '4px', display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', fontFamily: 'Roboto', borderBottom: '1px solid #d1d5db' }} key={charge._id}>
                  <Text style={{ width: '100%', fontSize: '0.4rem', textAlign: 'center' }}>
                    #{charge._id.slice(-5)}
                  </Text>
                  <Text style={{ width: '100%', fontSize: '0.4rem', textAlign: 'center' }}>
                    {formatCurrencyLocal(charge.total_amount)}
                  </Text>
                  <Text style={{ width: '100%', fontSize: '0.4rem', textAlign: 'center' }}>
                    {formatCurrencyLocal(charge.amount_one)}
                  </Text>
                  <Text style={{ width: '100%', fontSize: '0.4rem', textAlign: 'center' }}>
                    {formatCurrency(charge.amount_two)}
                  </Text>
                  <Text style={{ width: '100%', fontSize: '0.4rem', textAlign: 'center' }}>
                    {formatCurrencyLocal(charge.amount_three)}
                  </Text>
                  <Text style={{ width: '100%', fontSize: '0.4rem', textAlign: 'center' }}>
                    {formatDate(charge.settlement_date)}
                  </Text>
                  <Text style={{ width: '100%', fontSize: '0.4rem', textAlign: 'center' }}>
                    {paidStatusTranslations[charge.status]}
                  </Text>
                  <Text style={{ width: '100%', fontSize: '0.4rem', textAlign: 'center' }}>
                    {formatCurrencyLocal(charge.pending_amount)}
                  </Text>
                  <Text style={{ width: '100%', fontSize: '0.4rem', textAlign: 'center' }}>
                    {charge.product.id.name}
                  </Text>
                </View>
              ))}
